...
Учтите, что Kafka Engine обеспечивает at-least-once гарантию доставки.  Поэтому важно правильно выбрать агрегационные функции, так чтобы при вставке обеспечивалась идемпотентность, и повторная операция не портила состояние. 
